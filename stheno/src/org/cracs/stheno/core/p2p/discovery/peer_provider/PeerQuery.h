...
class PeerQuery : public DiscoveryQuery {
public:
    
    static const UInt ROUTING_QUERY;

    PeerQuery(UUIDPtr& uuid);
    PeerQuery(DiscoveryQuery* query);
    PeerQuery(const PeerQuery& orig);
    virtual ~PeerQuery();
