...
//
// The RDR::swap_X and RDR::swap_X_array routines are broken
...
ACE_INLINE void
RDR::swap_2 (const char *orig, char* target)
{
#if defined (ACE_HAS_INTRINSIC_BYTESWAP)
...
ACE_INLINE void
RDR::swap_4 (const char* orig, char* target)
{
#if defined (ACE_HAS_INTRINSIC_BYTESWAP)
  // Take advantage of MSVC++ compiler intrinsic byte swapping
  // function.
...
ACE_INLINE void
RDR::swap_8 (const char* orig, char* target)
{
#if defined (ACE_HAS_INTRINSIC_BYTESWAP)
  // Take advantage of MSVC++ compiler intrinsic byte swapping
  // function.
...
ACE_INLINE void
RDR::swap_16 (const char* orig, char* target)
{
...
ACE_INLINE size_t
RDR::first_size (size_t minsize)
{
  if (minsize == 0)
    return DEFAULT_BUFSIZE;

  size_t newsize = DEFAULT_BUFSIZE;
  while (newsize < minsize)
    {
      if (newsize < EXP_GROWTH_MAX)
        {
...
ACE_INLINE size_t
RDR::next_size (size_t minsize)
{
  size_t newsize =
    RDR::first_size (minsize);
