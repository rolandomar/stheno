 ...
 *  @file   RDR.h
 *
 *  $Id: RDR.h 76824 2007-01-31 19:04:39Z elliott_c $
 *
...
typedef ACE_CString String; //UTF-8 string
//typedef std::string String; //UTF-8 string

/*UInt STRING_SIZE(String* str){
    if(str == 0){
        return LONG_SIZE;
    }else{
        return LONG_SIZE+str->length();
    }
}*/
...
#define RDR_LONGLONG_INITIALIZER {0,0}
#else
#define RDR_LONGLONG_INITIALIZER 0
...
#define   RDR_LONG_DOUBLE_INITIALIZER 0
#define   RDR_LONG_DOUBLE_ASSIGNMENT(LHS, RHS) LHS = RHS
#else
#define NONNATIVE_LONGDOUBLE
#define   RDR_LONG_DOUBLE_INITIALIZER {{0}}
#define   RDR_LONG_DOUBLE_ASSIGNMENT(LHS, RHS) LHS.assign (RHS)
 ...
 * @class RDR
 *
 * @brief Keep constants and some routines runtime to both Output and
 * Input CDR streams.
 */
class RDR {
public:
    // = Constants defined by the RDR protocol.
    ...
    /// Align the packet block to RDR::MAX_ALIGNMENT,
    ...
    /// Copy a packet block chain into a single packet block,
    /// preserving the alignment of the first packet block of the
    /// original stream, not the following packet blocks.
    ...
    static ACE_Message_Block* consolidate(ACE_Message_Block* src, size_t size);
...
#if !defined (RDR_GIOP_MAJOR_VERSION)
#define RDR_GIOP_MAJOR_VERSION 1
#endif /*RDR_GIOP_MAJOR_VERSION */

#if !defined (RDR_GIOP_MINOR_VERSION)
#define RDR_GIOP_MINOR_VERSION 2
#endif /* RDR_GIOP_MINOR_VERSION */
};
