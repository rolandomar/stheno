...
/*ACE_RCSID (ace,
           CDR_Base,
           "$Id: CDR_Base.cpp 78245 2007-05-02 12:50:09Z johnnyw $")*/
...
size_t RDR::wchar_maxbytes_ = sizeof (WChar);
size_t RDR::wchar_maxbytes(){
    return wchar_maxbytes_;
}
...
void
RDR::swap_2_array (char const * orig, char* target, size_t n)
{
...
void
RDR::swap_4_array (char const * orig, char* target, size_t n)
{
  // ACE_ASSERT (n > 0); The caller checks that n > 0
...
//
void
RDR::swap_8_array (char const * orig, char* target, size_t n)
{
  // ACE_ASSERT(n > 0); The caller checks that n > 0
...
void
RDR::swap_16_array (char const * orig, char* target, size_t n)
{
  // ACE_ASSERT(n > 0); The caller checks that n > 0
...
void
RDR::mb_align (ACE_Message_Block *mb)
{
#if !defined (RDR_IGNORE_ALIGNMENT)
  char * const start = ACE_ptr_align_binary (mb->base (),
                                             MAX_ALIGNMENT);
#else
  char * const start = mb->base ();
#endif /* RDR_IGNORE_ALIGNMENT */
  mb->rd_ptr (start);
  mb->wr_ptr (start);
}

int
RDR::grow (ACE_Message_Block *mb, size_t minsize)
{
  size_t newsize =
    RDR::first_size (minsize + MAX_ALIGNMENT);
  ...
  // Do the equivalent of RDR::mb_align() here to avoid having
...
size_t
RDR::total_length (const ACE_Message_Block* begin,
...
ACE_Message_Block* RDR::consolidate(ACE_Message_Block* src,size_t size){
    return 0;
}

void
RDR::consolidate (ACE_Message_Block *dst,
                      const ACE_Message_Block *src)
{
  ...
  size_t newsize =
    RDR::first_size (RDR::total_length (src, 0)
                         + MAX_ALIGNMENT);
  dst->size (newsize);  
  ...
  ptrdiff_t srcalign =
    ptrdiff_t(src->rd_ptr ()) % MAX_ALIGNMENT;
  ptrdiff_t dstalign =
    ptrdiff_t(dst->rd_ptr ()) % MAX_ALIGNMENT;
  ...
  for (const ACE_Message_Block* i = src;
    ...
    ACE_OS::memset (dst->wr_ptr(), 0, dst->space());
}

#if defined (NONNATIVE_LONGLONG)
bool
RDR::RToLongLong::operator== (const RDR::RToLongLong &rhs) const
{
...
#if defined (NONNATIVE_LONGDOUBLE)
RDR::LongDouble&
RDR::LongDouble::assign (const RDR::LongDouble::NativeImpl& rhs)
{
  ...
  if (sizeof (rhs) == 8)
    {
      ...
      char rhs_ptr[16];
      RDR::swap_8 (reinterpret_cast<const char*> (&rhs), rhs_ptr);
#else
...
RDR::LongDouble&
RDR::LongDouble::assign (const RDR::LongDouble& rhs)
{
  if (this != &rhs)
    *this = rhs;
  return *this;
...
bool
RDR::LongDouble::operator== (const RDR::LongDouble &rhs) const
{
...
RDR::LongDouble::operator RDR::LongDouble::NativeImpl () const
{
  RDR::LongDouble::NativeImpl ret = 0.0;
...
RDR::Float::Float (void)
{
}

RDR::Float::Float (const float & /* init */)
{
}

RDR::Float &
RDR::Float::operator= (const float & /* rhs */)
{
  return *this;
}

bool
RDR::Float::operator!= (const RDR::Float & /* rhs */) const
{
  return false;
}
#endif /* _UNICOS */
