...
int main(int argc, char** argv) {
    ...
    int retval = dll.open("libleaf_plugin");
...

    {
        ...
        try {
            ...
            printf("Leaf: Runtime exception=%s\n", ex.message().c_str());
            runtime->close();
            delete runtime;
            exit(-1);
        ...
        resourceFile << "resource_leaf_" << testNO << "_" << runNO << "_" << runtimeUUID->toString().c_str() << ".log";
        ResourceMonitor rm;
