...
#ifndef ActuatorServiceSvcHandler_H
#define	ActuatorServiceSvcHandler_H
...
class ActuatorServiceSvcHandler : public PacketChannel<SthenoStreamPacket, SthenoStreamHeader,
ACE_SOCK_Stream, ACE_MT_SYNCH> {
public:
    typedef ACE_Strong_Bound_Ptr<ActuatorServiceSvcHandler,
    ACE_Recursive_Thread_Mutex> channel_ptr;

    static const int MAX_OPEN_TIMEOUT_MS = 2000;

    ActuatorServiceSvcHandler(ActuatorService* streamServer,
            ExecutionModelPtr* ec = 0,
            NetReservation* reserve = 0,
            NetQoS* qos = 0,
            CloseListener<ACE_SOCK_Stream, ACE_MT_SYNCH>* closeListener = 0);

    virtual ~ActuatorServiceSvcHandler();
...
#endif	/* ActuatorServiceSvcHandler_H */
